...
### Milestone 1: Architectural Decoupling (The Strategy Pattern) — ✅ Completed
...
- [x] **Define Abstract Base Classes (ABCs):** Implement a `BOMParserStrategy` interface that defines `parse()` and `can_handle()` methods.
- [x] **Concrete Implementations:** Create specific classes for `PDFParserStrategy`, `CSVParserStrategy`, and `ManualInputStrategy`.
- [x] **Context Manager:** Build a `BOMParserContext` to inspect file signatures and instantiate the correct strategy at runtime.

### Milestone 2: Linguistic Upgrade (From Regex to Grammars) — ✅ Completed

Moving from Regular Languages to Context-Free Languages to handle the structural complexity of non-passive components.

- [x] **Integrate Parser Combinators:** Use `pyparsing` to define atomic primitives like `digit`, `multiplier`, and `unit`.
- [x] **Recursive RKM Parsing:** Replace the RKM regex with a composite parser that handles variable decimal placement (e.g., $4k7$, $R22$, $0R1$) without greedy-matching false positives.
- [x] **Functional Transformation:** Use parse actions within parsers to normalize values to `Decimal` during the extraction phase rather than post-processing.

### Milestone 3: The Physics Layer (Precision and Units) — ✅ Completed

Eliminating floating-point artifacts and manual unit mapping.

- [x] **Numerical Precision:** Replace all `float` usage with the `Decimal` type to ensure exact precision across the $10^{18}$ dynamic range found in electronics.
- [x] **Unit Integration:** Use the `Pint` library to handle unit aliases and prefixes. This allows the system to treat $10uF$, $10\mu F$, and $10 \text{ microfarads}$ as identical objects.
- [x] **Dimensional Analysis:** Implement checks to prevent comparing orthogonal units (e.g., checking a voltage rating against a resistance value).
...
- [ ] **Confidence Intervals:** Engine assigns a confidence score $C \in [0.0, 1.0]$ to each parsed component.
